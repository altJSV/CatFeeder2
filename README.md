...
Потребляемы ток при максимальной нагрузке может достигать до 1.5А, поэтому необходимо подключение качественного блока питания. USB зарядка на 2.1А с этим неплохо справляется. 
